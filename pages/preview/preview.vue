<template>
	<view class="preview">
		<swiper ref="swiperRef" circular :current="currentIndex" @change="swiperChange" v-if="isReady">
			<swiper-item v-for="item in classList" :key="item._id">
				<image :src="item.picurl" mode="aspectFill" @click="changeMaskState"></image>
			</swiper-item>
		</swiper>
					...
					<view class="content">
						...
						<view class="row">
							...
							<view class="rate-box value">
								...
								<text selectable class="score">{{ currentInfo?.score || 0 }}分</text>
							</view>
						</view>
						<view class="row">
						...
						<view class="row">
							...
							<view class="tabs value">
								<text selectable class="tab" v-for="item in currentInfo?.tabs"
									:key="item">{{ item }}</text>
							</view>
						</view>
		...
		<uni-popup ref="scorePopup" :is-mask-click="false">
			<view class="scorePopup">
				...
				<view class="footer">
					<button size="mini" plain :disabled="!userScore || isSubmitting" @click="submitScore">
...
import { log } from "console";
	...
	onLoad(async (e) => {
		...
		// classList.value=storageClassList;
	...
	const submitScore = async () => {
		...
		try {
			...
			// 更新本地缓存的评分计数
			const cachedCount = await storage.get("userScoreCount") || 0;
